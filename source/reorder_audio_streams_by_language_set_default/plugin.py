...
from reorder_audio_streams_by_language_set_default.lib.ffmpeg import Parser, Probe, StreamMapper

# Configure plugin logger
logger = logging.getLogger("Unmanic.Plugin.reorder_audio_streams_by_language_set_default")
...
class PluginStreamMapper(StreamMapper):
    ...
    def check_default_disposition(self):
        """
    ...
    def streams_to_be_reordered(self):
        ...
        if self.search_string_stream_mapping and self.unmatched_stream_mapping:
            ...
            # First check if the default disposition needs to be set
            needs_default_flag = self.check_default_disposition()
            if needs_default_flag:
                logger.info("File needs processing: default disposition not set on first matching audio stream")
                return True
